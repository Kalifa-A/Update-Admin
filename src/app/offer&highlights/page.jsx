...
function CategoryTableDemo() {
  ...
  return (
    <>
      <Table>
       ...
       <TableBody>
  {/* Loading row: shown when loading */}
  {loading && (
    <TableRow>
  ...
  {!loading && (!offerDetails || offerDetails.length === 0) && (
    <TableRow key="no-offers">
  ...
  {!loading &&
    currentPageData.map((each, index) => {
      ...
      const safeDate = each.createdAt ? new Date(each.createdAt) : null;
      const dateStr = safeDate && !isNaN(safeDate) ? safeDate.toISOString().split("T")[0] : "N/A";
      ...
      return (
        ...
        >
          ...
          <TableCell>{each.offerCategory ?? "N/A"}</TableCell>
          <TableCell>{each.offerType ?? "N/A"}</TableCell>
          ...
          <TableCell>
            <Select
              ...
              onValueChange={async (value) => {
                try {
                  // Local UI update using offerDetails setter
                  const updated = [...(offerDetails || [])];
                  // find actual item index in full list if currentPageData is a slice
                  const globalIndex = offerDetails.findIndex((o) => o._id === each._id);
                  if (globalIndex !== -1) {
                    updated[globalIndex] = { ...updated[globalIndex], status: value };
                    setOfferDetails(updated);
                  }

                  // Backend update
                  const res = await fetch(
                    `http://localhost:5000/orders/${each._id}/status`,
                    {
                      method: "PUT",
                      headers: {
