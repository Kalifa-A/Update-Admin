...
export default function ReturnPage() {
    ...
    const fetchReturn = async () => {
      try {
        ...
        if (!ret) {
          ...
          // ensure returnItems is at least an empty array
          const safeRet = { returnItems: [], ...ret };
          setReturnData(safeRet);
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      const res = await fetch(`https://thajanwar.onrender.com/api/returns/${retId}/status`, {
        method: "PUT",
      ...
      // If API returns only partial, merge into existing state defensively:
      setReturnData((prev) => ({ ...(prev || {}), ...(updatedReturn || {}), status: newStatus }));
                    ...
                    {(returnData.returnItems || []).map((item, idx) => (
                      <tr key={idx}>
                        <td className="flex items-center gap-2">
                          ...
                          />
                          <span className="text-justify">{(item.name || "").slice(0, 10)}</span>
                        </td>
                        <td>{item.quantity}</td>
