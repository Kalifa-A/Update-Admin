...
const OrderDetails = () => {
  ...
  useEffect(() => {
    // Depend on `id` so we re-run when it changes
    if (!id) {
  ...
  const handleStatusChange = async (e) => {
    ...
    try {
      ...
      // Optionally update local `order` object so UI stays consistent
  ...
  const deliveryCharge = 100; // keep as-is or use order.deliveryCharge if available
