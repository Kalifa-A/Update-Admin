...
const normalizeCoupon = (raw) => {
  ...
  const deleteFromServer = async (id) => {
    const attempts = [
      { method: "DELETE", url: `https://thajanwar.onrender.com/api/coupons/${id}` },
      { method: "DELETE", url: `https://thajanwar.onrender.com/api/coupons/delete/${id}` },
      { method: "DELETE", url: `https://thajanwar.onrender.com/api/coupons/list/${id}` },
      // some servers use POST to delete
      { method: "POST", url: `https://thajanwar.onrender.com/api/coupons/delete/${id}`, body: null },
      { method: "POST", url: `https://thajanwar.onrender.com/api/coupons/${id}/delete`, body: null },
      // some servers accept a POST to /api/coupons with body { _method: 'DELETE', id }
      {
        method: "POST",
        url: `https://thajanwar.onrender.com/api/coupons`,
        body: JSON.stringify({ _method: "DELETE", id }),
        headers: { "Content-Type": "application/json" },
      },
      // last resort: POST to /api/coupons/delete with JSON id
      {
        method: "POST",
        url: `https://thajanwar.onrender.com/api/coupons/delete`,
    ...
    for (const attempt of attempts) {
      const { method, url, body, headers } = attempt;
      try {
        console.debug("Trying delete:", { method, url, body });
        ...
        console.debug("Delete attempt result:", { method, url, status: res.status, body: parsed });

        // success (2xx)
        if (res.ok) return { url, res, data: parsed };

        // if 404/405 — try next (endpoint not supported)
        if (res.status === 404 || res.status === 405) {
          console.warn(`Endpoint ${url} returned ${res.status} — trying next fallback`);
          continue;
        }

        // other non-ok status -> return this as failure info
        return { url, res, data: parsed };
      } catch (err) {
        console.warn(`Network error trying ${method} ${url}:`, err);
        continue;
      }
    }
  ...
  const handleDelete = async (id, code) => {
    ...
    try {
      const result = await deleteFromServer(id);

      if (!result) {
        // ALL attempts failed to reach the server (network/CORS)
        const msg = `All DELETE attempts failed for id: ${id}. See console for attempt logs.`;
        console.error(msg);
      ...
      if (!res.ok) {
        // server responded but rejected delete
        const debug = {
          attemptedUrl: url,
          status: res.status,
        ...
        console.error("Delete rejected by server:", debug);
        alert(
          `Delete rejected by server (${res.status}).\n\nCopy this and share with backend dev:\n\n` +
